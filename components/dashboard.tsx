...
const generateSampleData = (roomNumber: string, date: Date) => {
  const data = [];
  for (let i = 0; i < 24; i++) {
    data.push({
      time: `${i}:00`,
      temperature:
        Math.round(
          (20 + Math.random() * 5 + Math.sin((i / 24) * Math.PI * 2) * 2) * 10
        ) / 10,
      humidity:
        Math.round(
          (15 + Math.random() * 8 + Math.cos((i / 24) * Math.PI * 2) * 2) * 10
        ) / 10,
    });
  }
  return data;
  ...
  return (
    <div className="container mx-auto p-4">
        ...
        <Popover>
          ...
          <PopoverContent className="w-auto p-0" align="start">
            <Calendar
              ...
              } // Ensuring that it's always a Date
              disabled={(date) =>
      ...
      <Card className="mb-8">
        <CardHeader>
          <CardTitle>
            Room {selectedRoom} - {format(selectedDate, "PPP")}
          </CardTitle>
          <CardDescription>
            Temperature and Humidity over 24 hours
          </CardDescription>
        </CardHeader>
        <CardContent>
          <ChartContainer
